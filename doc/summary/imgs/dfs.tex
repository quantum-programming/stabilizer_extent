...
\begin{tikzpicture}[
    ...
    \foreach \pos/\a/\b/\c/\d/\x/\y/\z/\thres/\thresColor in {
            ...
            \renewcommand{\baselinestretch}{1.0}
            \node[anchor=north west,scale=1.2] at (\pos+1.1,-3.7) {$\phantom{Q}\makebox[0pt][r]{c}=\mqty[\x\\?]$};
            \node[anchor=north west,scale=1.2] at (\pos+1.1,-4.9) {$Q=\mqty[\y&\z\\&?]$};
    ...
    \node[align=center,red] at (13,-9.5) {\LARGE{Branch Cut}\\\large{(skip this subtree)}};
    ...
    Evaluate: \\[1em]
    ${\displaystyle \abs{\sum_{x=0}^{2^n-1} -1^{x^\top Q x} i^{c^\top x} P_x}}$
    };
