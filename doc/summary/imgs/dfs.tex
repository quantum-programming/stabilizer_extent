...
\begin{document}
\begin{tikzpicture}[
        myBlock1/.style={draw=cA,fill=cA!50,thick},
        myBlock2/.style={draw=cB,fill=cB!50,thick},
        myBlock3/.style={draw=cC,fill=cC!50,thick},
        myNode/.style={align=right,midway},
    ]
    \foreach \i/\bit in {0/00,1/01,2/10,3/11}{
    \renewcommand{\baselinestretch}{1.5}
    \draw[myBlock1](0,-3/4*\i) rectangle (1,-3/4*\i-0.75) node[myNode] {$+b_{\bit}$};
    ...
    \foreach \pos/\a/\b/\c/\d/\x/\y/\z/\thres/\thresColor in {
            ...
            \renewcommand{\baselinestretch}{1.5}
            \draw[myBlock2](\pos,-4) rectangle (\pos+1,-5.5) node[myNode] {$\a b_{00}$\\$\c b_{10}$};
            \draw[myBlock2](\pos,-5.5) rectangle (\pos+1,-7) node[myNode] {$\b b_{01}$\\$\d b_{11}$};
            \renewcommand{\baselinestretch}{1.0}
    ...
    \foreach \i/\x/\a/\b/\c/\d in {
            ...
            node[myNode] {$\a b_{00}$\\$\c b_{10}$\\$\b b_{01}$\\$\d b_{11}$};
            \renewcommand{\baselinestretch}{1.0}
    ...
    \node[font=\LARGE,cA] at (-1,-1.5) {$2^2$};
    \node[font=\LARGE,cB] at (-1,-5.5) {$2^1$};
    \node[font=\LARGE,cC] at (-1,-9.5) {$2^0$};

    \node[font=\LARGE,align=left] at (7,-1.5) {
        Define:\\
        $n=k=2, R=I_n, t=0, b_{x} \defeq \braket{x}{\psi}$
    };
    \node[font=\LARGE,align=left] at (15,-1.5) {
    Evaluate: \\
    $\sum_{x=0}^{2^n-1} -1^{x^\top Q x} i^{c^\top x} b_x$
    };
