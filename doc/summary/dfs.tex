...
\begin{document}
\begin{tikzpicture}[
        myBlock1/.style={draw=blue,fill=blue!50,thick},
        myBlock2/.style={draw=orange,fill=orange!50,thick},
        myBlock3/.style={draw=ForestGreen,fill=ForestGreen!50,thick},
    ...
    \foreach \pos/\a/\b/\c/\d/\x/\y/\z/\thres/\thresColor in {
            ...
            \renewcommand{\baselinestretch}{1.5}
            \draw[myBlock2](\pos,-4) rectangle (\pos+1,-5.5) node[myNode] {$\a P_{00}$\\$\b P_{01}$};
            \draw[myBlock2](\pos,-5.5) rectangle (\pos+1,-7) node[myNode] {$\c P_{10}$\\$\d P_{11}$};
            \renewcommand{\baselinestretch}{1.0}
            \node[anchor=north west] at (\pos+1.2,-4) {$\phantom{Q}\makebox[0pt][r]{c}=\mqty[\x\\?]$};
            \node[anchor=north west] at (\pos+1.2,-5) {$Q=\mqty[\y&\z\\&?]$};
            \node[anchor=north west] at (\pos+1.2,-6.5) {threshold$ \leq \thresColor{\thres}$};
        }
    ...
    \foreach \i/\a/\b/\c/\d in {
            ...
            \renewcommand{\baselinestretch}{1.5}
            \draw[myBlock3] (\i,-8) rectangle (\i+1,-11)
            node[myNode] {$\a P_{00}$\\$\b P_{01}$\\$\c P_{10}$\\$\d P_{11}$};
            \renewcommand{\baselinestretch}{1.0}
        }
    \foreach \i/\a/\b/\c/\d in {
            0/+/+/+/-/,
            ...
            \renewcommand{\baselinestretch}{1.5}
            \draw[myBlock3] (\i+5,-8) rectangle (\i+6,-11)
            node[myNode] {$\a P_{00}$\\$\b P_{01}$\\$\c P_{10}$\\$\d P_{11}$};
            \renewcommand{\baselinestretch}{1.0}
        }
    \foreach \i/\a/\b/\c/\d in {
            ...
            \renewcommand{\baselinestretch}{1.5}
            \draw[myBlock3] (\i+16,-8) rectangle (\i+17,-11)
            node[myNode] {$\a P_{00}$\\$\b P_{01}$\\$\c P_{10}$\\$\d P_{11}$};
            \renewcommand{\baselinestretch}{1.0}
        }
    ...
    \newcommand{\Arrows}[3]{
        \begin{scope}[xshift=#1.0cm, yshift=-#2.0cm]
            \draw[thick,->] (0.5,-0.0) -- (0.5,-1.0);
            \draw[thick,->] (1.5,-0.5) -- (1.5,-1.0);
            \draw[thick,->] (2.5,-0.5) -- (2.5,-1.0);
            \draw[thick,->] (3.5,-0.5) -- (3.5,-1.0);
    ...
    \node[align=center,red] at (13,-9.5) {\LARGE{Branch Cut}\\(skip this subtree)};

    \node[anchor=center,font=\LARGE] at (10,-5.5) {$\cdots$};
    \node[anchor=center,font=\LARGE] at (10,-9.5) {$\cdots$};

    \node[font=\LARGE,blue] at (-1,-1.5) {$2^2$};
    \node[font=\LARGE,orange] at (-1,-5.5) {$2^1$};
    \node[font=\LARGE,ForestGreen] at (-1,-9.5) {$2^0$};

    \node[font=\LARGE,align=left] at (7,-1.5) {
        Define:\\
        $n=k=2, R=I_n, t=0, P_{x} \defeq \braket{x}{\psi}$
    };
    \node[font=\LARGE,align=left] at (15,-1.5) {
    Evaluate: \\
